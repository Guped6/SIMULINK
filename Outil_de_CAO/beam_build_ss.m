...
beta = 0%.000001;          % Coefficient de Rayleigh pour l'amortissement interne (forces dans le matériel)
...
% Méthode de Rayleigh (Equation 35 du papier)
C_g = alpha * M_g + beta * K_g;

%% 4. Application des conditions aux limites (Encastrement)
% À l'encastrement (noeud 1), le déplacement et la rotation sont nuls.
% On retire les 2 premiers DDL du système.
ddl_libres = 3:ndof;
...
% Vecteur d'état x = [d1 d2 d3 d1_dot d2_dot d3_dot]'
% Matrice A_ss (Equation 48 du papier)
A_ss = [zeros(n_modes, n_modes), eye(n_modes);
    -inv(M_bar)*K_bar,       -inv(M_bar)*C_bar];        %Variables: M_bar,K_bar,C_bar

% Vecteur de force externe
F_ext = zeros(length(ddl_libres), 1);
idx_force_reduit = ddl_act - 2; % -2 à cause des conditions aux limites enlevées
F_ext(idx_force_reduit) = 1; % Position de l'application de la force

% Matrice B_ss
B_ss = [zeros(n_modes, 1);
     inv(M_bar) * (Phi_tronque' * F_ext)];              %Variables: M_bar, Phi_tronque',F_ext

% Matrice C_ss (Sortie: position du bout et position de l'actionneur)
% La sortie y(t) est liée aux coordonnées généralisées
...
%% 8. Simulation et extraction des données temporelles
...
% Si tu veux réactiver ta force constante plus tard, tu l'ajouteras ici :
% F_constante = 0;
% F_ext = F_ext + F_constante;

[y_lsim, t_out] = lsim(sys, F_ext, t);

pos_bout_m2 = y_lsim(:, 1);
pos_actionneur_m2 = y_lsim(:, 2);

%% 9. Affichage des résultats
if plot_position;
figure;
% On affiche la méthode 2 (lsim), mais les deux donneront le même résultat
plot(t_out, pos_bout_m2, 'b', t_out, pos_actionneur_m2, 'r');
grid on;
title('Position selon le temps (Impulsion de 0.1 N.s)');
xlabel('Temps [s]');
...
if faire_fft
    ...
    h_act = pos_actionneur_m2;
    L_sig_act = length(h_act); % Recalculer la longueur au cas où
